...
map_server <- function(IP, OP, RV, SESSION){
  ...
  map_data <- reactive({
    ...
    temp_all_data
  }) |>bindEvent(RV$bases(),IP$co_select_indicator,IP$co_select_year)
  ...
  pallet <- reactive({
    ...
    temp_all_pallet
  }) |>bindCache(RV$bases(),IP$co_select_indicator,map_data())
  ...
  }) |>bindCache(IP$map_groups,IP$co_select_indicator,IP$co_select_year,IP$l,map_data())
